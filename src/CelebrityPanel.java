...
public class CelebrityPanel extends JPanel implements ActionListener {
  ...
  private void updateScreen() {
    ...
    cluesAndResponses.append("\nYou guessed: ").append(guess).append("\n");

    if (controller.processGuess(guess)) {
      updateSuccess(cluesAndResponses);
    } else {
      updateFailure(cluesAndResponses);
    }

    checkGameCompletion();

    clueArea.setText(cluesAndResponses.toString());
  }

  private void updateSuccess(StringBuilder cluesAndResponses) {
    clueArea.setBackground(Color.cyan);
    cluesAndResponses.append("\n").append(success);
    cluesAndResponses.append(controller.sendClue());
  }

  private void updateFailure(StringBuilder cluesAndResponses) {
